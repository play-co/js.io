...
this.isConvex = function () {
  var x = this.x, y = this.y, length = x.length;
  // Polygon is non-degenerate
  if (length < 3) return false;
  
  var ax = x[0]-x[length-1], ay = y[0]-y[length-1],
      bx = x[1]-x[0], by = y[1]-y[0];
  ...
  for (k = 1; k < length-1; k++){
    ax = x[k]-x[k-1], ay = y[k]-y[k-1];
    bx = x[k+1]-x[k], by = y[k+1]-y[k];
    ...
    ax = x[length-1]-x[length-2], ay = y[length-1]-y[length-2];
    bx = x[0]-x[length-1], by = y[0]-y[length-1];
    theta = Math.asin((by*ax-bx*ay) / (norm([ax,ay])*norm([bx,by])));
