...
function checkProp (value, schema, path, i, errors) {
  ...
  path += path ? typeof i == 'number' ? '[' + i + ']' : typeof i == 'undefined' ?
    '' : '.' + i : i;
    ...
    if (value !== null) {
      if (Array.isArray(value)) {
        ...
        if (schema.minItems && value.length < schema.minItems) {
          addError('There must be a minimum of ' + schema.minItems +
            ' in the array');
        }
        if (schema.maxItems && value.length > schema.maxItems) {
          addError('There must be a maximum of ' + schema.maxItems +
            ' in the array');
        }
      ...
      if (schema.maxLength && typeof value == 'string' && value.length > schema
        .maxLength) {
      ...
      if (typeof schema.maxDecimal == 'number' && value.toString().match(new RegExp(
          '\\.[0-9]{' + (schema.maxDecimal + 1) + ',}'))) {
        addError('may only have ' + schema.maxDecimal +
...
function checkObj (instance, objTypeDef, path, additionalProp, errors) {
  if (typeof objTypeDef == 'object') {
    ...
    for (var i in objTypeDef) {
      if (objTypeDef.hasOwnProperty(i) && !(i.charAt(0) == '_' && i.charAt(1) ==
          '_')) {
  ...
  for (i in instance) {
    if (instance.hasOwnProperty(i) && !(i.charAt(0) == '_' && i.charAt(1) ==
        '_') && objTypeDef && !objTypeDef[i] && additionalProp === false) {
      errors.push({
        property: path,
      ...
      switch (type) {
        ...
        case 'array':
          if (!Array.isArray(value)) {
            return [{
              property: path,
              message: 'expected an array, but ' + actualType +
              ' found instead'
            }];
          }
          break;
        ...
        default:
          if (actualType != type) {
            return [{
              property: path,
              message: 'expected ' + type + ', but ' + actualType +
              ' found instead'
            }];
          }
          break;
