...
exports = (function () {
  ...
  var Sizzle = function (selector, context, results, seed) {
    ...
    if (parts.length > 1 && origPOS.exec(selector)) {
      ...
      if (!seed && parts.length > 1 && context.nodeType === 9 && !contextXML && Expr.match.ID.test(parts[0]) && !Expr.match.ID.test(
          parts[parts.length - 1])) {
        var ret = Sizzle.find(parts.shift(), context, contextXML);
        context = ret.expr ? Sizzle.filter(ret.expr, ret.set)[0] : ret.set[
          0];
      }
  ...
  var Expr = Sizzle.selectors = {
      ...
      '+': function (checkSet, part, isXML) {
        ...
        for (var i = 0, l = checkSet.length, elem; i < l; i++) {
          if (elem = checkSet[i]) {
            ...
            checkSet[i] = isPartStrNotTag || elem && elem.nodeName ===
              part ? elem || false : elem === part;
          }
        }
    ...
    preFilter: {
      CLASS: function (match, curLoop, inplace, result, not, isXML) {
        ...
        for (var i = 0, elem;
          ...
          if (elem) {
            if (not ^ (elem.className && (' ' + elem.className + ' ').indexOf(
                match) >= 0)) {
              if (!inplace)
                { result.push(elem); }
            } else if (inplace) {
      ...
      CHILD: function (match) {
        if (match[1] == 'nth') {
          ...
          var test = /(-?)(\d*)n((?:\+|-)?\d*)/.exec(match[2] == 'even' &&
            '2n' || match[2] == 'odd' && '2n+1' || !/\D/.test(match[2]) &&
            '0n+' + match[2] || match[2]);
    ...
    filters: {
      ...
      button: function (elem) {
        return elem.type === 'button' || elem.nodeName.toUpperCase() ===
          'BUTTON';
      },
      input: function (elem) {
    ...
    filter: {
      PSEUDO: function (elem, match, i, array) {
        ...
        if (filter) {
          ...
          return (elem.textContent || elem.innerText || '').indexOf(match[
            3]) >= 0;
        } else if (name === 'not') {
          var not = match[3];
      ...
      ATTR: function (elem, match) {
        ...
        return result == null ? type === '!=' : type === '=' ? value ===
          ...
          '^=' ? value.indexOf(check) === 0 : type === '$=' ? value.substr(
            value.length - check.length) === check : type === '|=' ?
