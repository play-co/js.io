...
jsio.Class = function(parent, proto) {
    console.log('parent is', parent, 'proto', proto);
...
_require = function(context, path, pkg, what) {
	...
	if(!(pkg in modules)) {
		...
		var newRelativePath = segments.slice(0, segments.length - (result[1].match('__init__.js$') ? 0 : 1)).join('.');
